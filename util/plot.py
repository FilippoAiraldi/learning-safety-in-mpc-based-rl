...
def _plot_population(
    ...
    **kwargs,
    ...
    if method == "errorbar":
        ...
        y_std = y_std if y_std is not None else np.nanstd(y, axis=0)
        func(
            x, y_mean - y_std, y_mean + y_std, alpha=SMALL_ALPHA[PAPERMODE], color=color
        )
        ax.plot(
            x, y_mean, lw=lw, color=color, linestyle=linestyle, label=label, **kwargs
        )
    else:
        if y is not None:
            func(
...
def set_mpl_defaults(matlab_colors: bool = False, papermode: bool = False) -> None:
...
def constraint_violation(
    ...
    if fig is None:
        ...
        axs = [fig.add_subplot(G[i, j]) for i, j in product(range(nrows), range(ncols))]
    ...
    observations = np.transpose(jstack([jstack(a.env.observations) for a in agents]))
...
def learned_weights(
    ...
    norms = np.sqrt(
        np.square(
            np.ma.masked_invalid(jstack([agent.update_gradient for agent in agents]))
        ).sum(axis=-1)
    )
    ...
    for ax, name in zip(axs, weightnames):
        ...
        weights = jstack([np.squeeze(agent.weights_history[name]) for agent in agents])
        ...
        # plot
        _plot_population(
            ax, updates, weights, color=color, label=label, xlabel="Update", ylabel=lbl
        )
    _set_empty_axis_off(axs)
    return fig
...
def safety(
    ...
    _plot_population(
        ...
        label=label,
        xlabel="Episode",
        ylabel="Number of unsafe episodes",
        legendloc="upper left",
    )

    attr = "backtracked_betas"
    if agents is not None and all(a is not None and hasattr(a, attr) for a in agents):
...
def paperplots(
    agents: dict[str, list[AGENTTYPE]], save2tikz: bool = False
) -> list[Figure]:
    """
    ...
    def figure1() -> Figure:
        fig, ax = plt.subplots(1, 1, constrained_layout=True)
        baseline = np.nanmean(list(chain.from_iterable(a.env.cum_rewards for a in pk)))
        performances = [
            jstack([a.env.cum_rewards for a in agents]) for agents in learning_agents
        ]
    ...
    def figure2() -> Figure:
        ...
        print(
            "Unsafe episodes average reduction =", ", ".join(f"{p:.2f}%" for p in percs)
        )
    ...
    def figure3() -> Figure:
        fig, ax = plt.subplots(1, 1, constrained_layout=True)
        for agents, clr, lbl in zip(learning_agents[1:], colors[1:], labels[1:]):
    ...
    figs = [
        fcn() for k, fcn in locals().items() if k.startswith("figure") and callable(fcn)
    ]
    if save2tikz:
        _save2tikz(*figs)
    return figs
