...
def parse_args() -> argparse.Namespace:
    ...
    group.add_argument(
        ...
        help='If passed, trains the LSTD Q-learning agent WITHOUT the safety '
             'mechanism.')
    group.add_argument(
        '--safe-lstdq', '--safe_lstdq', action='store_true',
    ...
    group.add_argument('--n-jobs', '--n_jobs', type=int, default=-1,
    ...
    group.add_argument(
        '--replay-mem-size', '--replay_mem_size', type=int, default=1,
        help='How many epochs the replay memory can store.')
    group.add_argument(
        '--replay-mem-sample', '--replay_mem_sample', type=float, default=1.0,
        help='Size of the replay memory samples (percentage).')

    group = parser.add_argument_group(
        'GP parameters (used only for safe algorithm)')
    group.add_argument(
        '--gp-alpha', '--gp_alpha', type=float, default=1e-10,
        help='Estimated measurement noise of the GP data.')
    group.add_argument(
        '--gp-kernel', '--gp_kernel', choices=('RBF', 'Matern'),
        default='RBF', help='Type of GP kernel.')
    group.add_argument(
